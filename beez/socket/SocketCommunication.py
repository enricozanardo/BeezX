...
class SocketCommunication(Node):
    ...
    def node_message(self, connectedNode: Node, message: Message):
        ...
        if message.messageType == MessageType.DISCOVERY.name:
        ...
        elif message.messageType == MessageType.CREATED.name:
            # handle the CREATED
            logger.info(f"manage the message {message.messageType}")
        ...
        elif message.messageType == MessageType.REWARD.name:
            # handle the REWARD
            logger.info(f"manage the message {message.messageType}")
            challenge: Challenge = message.challenge
            self.beezNode.handleChallengeReward(challenge)
            ...
            logger.info(f"A CHALLENGETXCLOSED Message will be broadcasted!! {message.messageType}")
            logger.info(f"{type(message.challengeTx)}")
            challengeTransaction : ChallengeTX  = message.challengeTx
