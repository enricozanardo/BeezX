...
def wait_until_timeout(func, expected_value, interval, timeout):
    waiting_since = 0
    while waiting_since < timeout:
        value = func()
        LOGGER.info(f'Current test: {value}:{expected_value}')
...
def test_connect_two_nodes():
    ...
    try:
        ...
        def test_func():
            return seed_node.p2p.socket_connector.port
        wait_until_timeout(test_func, 8111, 5, TIMEOUT)

        def test_func():
            return seed_node.port
        wait_until_timeout(test_func, 8111, 5, TIMEOUT)

        def test_func():
            return seed_node.get_ip()
        wait_until_timeout(test_func, ip, 5, TIMEOUT)

        def test_func():
            return len(distinct_nodes(storage_node.p2p.all_nodes))
        ...
        wait_until_timeout(test_func, 1, 5, TIMEOUT)
        def test_func():
            return len(distinct_nodes(seed_node.p2p.all_nodes))
        wait_until_timeout(test_func, 1, 5, TIMEOUT)
    finally:
        clear_indices()
        seed_node.stop()
...
def test_connect_three_nodes():
    ...
    try:
        ...
        # def test_func():
        #     return len(distinct_nodes(storage_node.p2p.all_nodes))
        # wait_until_timeout(test_func, 2, 5, TIMEOUT)

        timeout = TIMEOUT
        waiting_since = 0
        while waiting_since < timeout:
            value = len(distinct_nodes(storage_node.p2p.all_nodes))
            LOGGER.info(f'Current test: {value}:2')
            if value == 2:
                break
            time.sleep(5)
        assert value == 2
        ...
        def test_func():
            return len(distinct_nodes(storage_node_2.p2p.all_nodes))
        wait_until_timeout(test_func, 2, 5, TIMEOUT)
        def test_func():
            return len(distinct_nodes(seed_node.p2p.all_nodes))
        wait_until_timeout(test_func, 2, 5, TIMEOUT)

        # wait_until_timeout(len(seed_node_all_connections), 2, 5, TIMEOUT)
...
#         waiting_for_api = 0
#         while not upload_successful and waiting_for_api < 120:
...
#         wait_until_timeout(len(storage_node.primary_chunks), 2, 5, 120)
#         wait_until_timeout(len(storage_node.backup_chunks), 2, 5, 120)
...
#         wait_until_timeout(chunk_parts, ["0.dap", "3.dap"], 5, 120)
#         wait_until_timeout(backup_chunk_parts, ["1.dap", "4.dap"], 5, 120)
#         # assert len(storage_node_2.primary_chunks) == 2
#         wait_until_timeout(len(storage_node_2.primary_chunks), 2, 5, 120)
#         chunk_parts = []
#         for chunk in storage_node_2.primary_chunks:
#             chunk_parts.append(chunk.split('-')[1])
#         # assert chunk_parts == ["1.dap", "4.dap"]
#         # assert len(storage_node_3.primary_chunks) == 1
#         wait_until_timeout(chunk_parts, ["1.dap", "4.dap"], 5, 120)
#         wait_until_timeout(len(storage_node_3.primary_chunks), 1, 5, 120)
#         chunk_parts = []
#         for chunk in storage_node_3.primary_chunks:
#             chunk_parts.append(chunk.split('-')[1])
#         # assert chunk_parts == ["2.dap"]
#         wait_until_timeout(chunk_parts, ["2.dap"], 5, 120)
#     finally:
#         clear_indices()
#         seed_node.stop()
#         storage_node.stop()
...
#         # test storage_node
#         # assert len(storage_node.primary_chunks) == 2
#         wait_until_timeout(len(storage_node.primary_chunks), 2, 5, TIMEOUT)
#         chunk_parts = []
#         backup_chunk_parts = []
#         for chunk in storage_node.primary_chunks:
#             chunk_parts.append(chunk.split('-')[1])
#         for chunk in storage_node.backup_chunks:
#             backup_chunk_parts.append(chunk.split('-')[1])
#         wait_until_timeout(sorted(chunk_parts), ["0.dap", "3.dap"], 5, TIMEOUT)
#         wait_until_timeout(sorted(backup_chunk_parts), ["1.dap", "2.dap", "4.dap"], 5, TIMEOUT)
...
#         wait_until_timeout(sorted(chunk_parts), ["1.dap", "2.dap", "4.dap"], 5, TIMEOUT)
#         wait_until_timeout(sorted(backup_chunk_parts), ["0.dap", "3.dap"], 5, TIMEOUT)
#     finally:
#         clear_indices()
#         seed_node.stop()
#         storage_node.stop()
