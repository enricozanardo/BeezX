...
def test_connect_three_nodes():
    ...
    try:
        clear_indices()
        ip = get_ip()
        seed_node = SeedNode(ip_address=ip, port=8211)
        seed_node.start_p2p()
        time.sleep(5)
        storage_node = BeezNode(ip_address=ip, port=5546, first_server_ip=ip, first_server_port=8211)
        storage_node.start_p2p()
        time.sleep(5)
        storage_node_2 = BeezNode(ip_address=ip, port=5548, first_server_ip=ip, first_server_port=8211)
